...
def open_settings_window() -> None:
    ...
    shell = ctk.CTkFrame(app, fg_color="#ffffff", corner_radius=18, border_width=1, border_color="#e5e5ea")
    shell.grid(row=0, column=0, padx=14, pady=14, sticky="nsew")
    ...
    if icon_png.exists():
        try:
            icon_image = ctk.CTkImage(light_image=Image.open(icon_png), size=(40, 40))
        except Exception:
            icon_image = None

    icon_box = ctk.CTkLabel(header, text="", image=icon_image, width=42, height=42, fg_color="#f5f5f7", corner_radius=12)
    ...
    status_badge.grid(row=1, column=0, padx=20, sticky="ew")

    body = ctk.CTkFrame(shell, fg_color="transparent")
    body.grid(row=2, column=0, padx=20, pady=(14, 0), sticky="ew")
    ...
    ctk.CTkLabel(body, text="复制格式", anchor="w", font=ctk.CTkFont(family=FONT_FAMILY, size=12, weight="bold")).grid(
        row=2, column=0, pady=(14, 0), sticky="ew"
    )
    ...
    compact_row.grid(row=5, column=0, pady=(14, 0), sticky="ew")
    ...
    ).grid(row=3, column=0, padx=20, pady=(18, 0), sticky="ew")

    actions = ctk.CTkFrame(shell, fg_color="transparent")
    actions.grid(row=4, column=0, padx=20, pady=(10, 0), sticky="ew")
    ...
    ctk.CTkLabel(
        ...
        font=ctk.CTkFont(family=FONT_FAMILY, size=11),
        text_color="#6e6e73",
    ).grid(row=5, column=0, padx=20, pady=(10, 16), sticky="ew")
